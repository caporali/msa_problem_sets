...
\smallskipm
The observations $2$ and $11$ are above the blue line (which corresponds to $\chi_9^2(\alpha_2)$), hence they can be confidently considered as multivariate outliers. This confirms what we previously noticed in the Chi-squared Q-Q plot. \newline
Note also that some observations lie in the strip delimited by the two lines. Nonetheless, we choose not to consider them as multivariate outliers, taking also in account what we observed in the previous point. \newline
...
<!-- todo (maybe): final remark from. -->
<!-- These two states were the last ones to join the federation. Of course we don't claim this to be causal. Nonetheless, the absense of contiguity with the other states definitely reduces the net flow with other states therefore reducing homogenizing factors. -->
...
<!-- \[
        \Sigma^{-1} = y I + k aa^T.
    \]
Observe that $P \deq aa^T$ is a projection matrix. Therefore, while in principle $\Sigma \Sigma^{-1} = I$ may yield no solution as we have a second degree polynomial in $P$ on l.h.s., as $P$ is idempotent, we have that l.h.s. is actually of first degree in $P$. \newline
We now find the values for $y$ and $k$. -->
    \[
        \Sigma^{-1} = y I + k aa^T.
    \]
and than find the values for $y$ and $k$. We obtain: 
    \begin{align*}
...
It can be of interest to asses whether these outliers were also multivariate outliers with respect to the original variables. Therefore, we also plot the corresponding squared Mahalanobis distances, with the quantiles of a $\chi^2_6$ of levels $\alpha = 0.95$ and $\alpha = 0.99$.
