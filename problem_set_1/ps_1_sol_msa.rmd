...
Clearly this method is not intended to be exact as it does not take into account possible heavy tails in the distributions nor numerous other degenerate behaviours. 
...
We can compute $\Sigma^{-1}$ in many other ways; for example we can suppose that $\Sigma^{-1}$ is of the same form $\Sigma$, i.e.
	\[
        \Sigma^{-1} = y I + k aa^Tm 
    \]
and than observe that $P \deq aa^T$ is such that 
	\[	
