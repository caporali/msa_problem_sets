...
setwd("c:/users/franc/documents/github/msa_problem_sets/problem_set_1")
...
Preliminary to our analysis, we rename some variables to avoid spaces, and add the variable \textit{Density} representing the population density.
\smallskip
\smallskip
...
In order to compute and visualize the correlation matrix we use the function \texttt{corrplot}. The plot is displayed at the beginning of the next page.
\smallskip
...
By analysing the correlation matrix, we can observe that:
\begin{itemize}
    \item 
    ...
    \item 
        the variables with the higher positive correlation ($0.7$) are \textit{Murder} and \textit{Illiteracy}, which is indeed a plausible relation;
\end{itemize}
...
It is important to emphasise that we do not assume in any way that our data after the scaling procedure have assumed a distribution $\mathcal{N}\left(0, 1\right)$.
Indeed, the use of the quantiles of a Gaussian is motivated by the fact that the particular bell shape of its density makes it a natural tool for identifying the most extreme values in any normalised distribution.
Clearly this method is not intended to be exact as it does not take into account possible heavy tails in the distributions nor other degenerate behaviours. 
There are various ways of identifying potential univariate outliers, for example by using built in tools such as box plots, however the procedure we have used allows us to make preliminary assumptions that can then be confirmed through other techniques.
...
In the following data frames (respectively \texttt{mat\_1} and \texttt{mat\_2}) the first column refers to the index of the corresponding observation, while the second one refers to the variable with respect to this observation is a potential univariate outlier.

```{r, echo = F}
rownames(mat_1)[c(2, 3, 5, 7, 8)] = c("NewYork", "Alaska_Income", "Alaska_Area", "NewJersey", "RhodeIsland")
```
```{r, render = lemon_print}
mat_1
```
...
rownames(mat_2)[c(2, 3, 5, 8, 9)] = c("NewYork", "Alaska_Income", "Alaska_Area", "NewJersey", "RhodeIsland")
```
```{r, render = lemon_print}
mat_2
```

\textit{Note}: In the tables above the names Alaska\_Income and Alaska\_Area both refer to observation 2 (i.e. Alaska). As it constitutes a potential univariate outlier for both variables, we opted for a relabelling to display them separately. 

## 1.3

We report below the box plots corresponding to each variable. We highlighted in red the potential univariate outliers found in point $1.2$.
...
\begin{itemize}
    ...
    \item 
        the potential outlier we identified for the variable \textit{Illiteracy} (observation $18$) does not show up in the corresponding box plot. This is plausible, since the variable's distribution seems to have very fat tails. For this reason we choose to do not consider this observation as an outlier;
    \item 
    ...
    \item 
        as for the remaining variables, the box plots generated many other potential univariate outliers, but the ones that we did not detect in the previous point are not in the outer tails (at least $98.75$th percentile) of the distribution. Hence, we will not think of them as outliers.
\end{itemize}
\smallskip
In conclusion, by looking at the box plots we infer that observations $2, 5, 21, 30, 32, 39$ and $43$ are potential univariate outliers.
...
\begin{itemize}
    \item 
        all the values corresponding to the variable \textit{Income} lie very close to the Q-Q line, except for the observation $2$, which was previously identified as an univariate outlier;
    \item 
        also the variable \textit{Life\_Exp} seems to have a distribution close to normal: the values are more spread out with respect to the ones corresponding to the variable \textit{Income}, but they still are very close to the blue line (which represents the linear relationship between the sample and the theoretical quantiles);
    \item 
        the variables \textit{Murder}, \textit{HS\_Grad} and \textit{Frost} have a very similar behaviour: most of the points lie near the Q-Q line, but they have a thinner right tail and a heavier left tail. Note the absence of univariate outliers;
    \item 
        also the variables \textit{Population}, \textit{Area} and \textit{Density} have very similar shapes, which are pretty far from being linear. All of them have heavy tails, which is also caused by the presence of more than one outlier;
    \item 
...
In conclusion, we can infer a gaussian behaviour only for the variables \textit{Income} and \textit{Life\_Exp}.
...
\begin{itemize}
    \item 
        the variables \textit{Population} and \textit{Density} follow the same non-gaussian behaviour that we observed before; as a matter of fact, their p-values are almost negligible;
    \item 
        as for the variable \textit{Income}, we can still argue that its distribution is gaussian. However, from the Q-Q plot we can observe the presence of more pronounced tails. Its p-value is still greater than $0.05$ yet a slightly lower than the previous one;
    \item 
        finally, by removing its outliers, the variable \textit{Area} seems to become gaussian: its p-value is very close to $0.05$ but there is a huge difference between the current and the previous p-value.
\end{itemize}

## 1.5

We report the scatter plot of the variables \textit{Area} \textit{vs} \textit{Population}, where have coloured all the potential univariate outliers.
...
From the plot we can observe that all the mass is roughly concentrated in the rectangle $[0, 15 \times 10^4]\times [0, 15 \times 10^3]$. 
Furthermore, unlike the other univariate outliers, only the points corresponding to the observations $2, 5, 32$ and $43$ seems to be really far from the others. Hence, we can identify them as bivariate outliers.
...
In order to identify the multivariate outliers, we can plot the vector of the squared Mahalanobis distances and then add some threshold lines corresponding to different levels of the theoretical quantiles of a $\chi_9^2$ (in particular, we used $\alpha_1 = 0.95$ and $\alpha_2 = \frac{(n - 0.5)}{n} = 0.99$ with $n = \texttt{nrow(st)}$).
\vspace{-1.5cm}
...
\smallskipm
The observations $2$ and $11$, corresponding to Alaska and Hawaii, are above the blue line (which corresponds to $\chi_9^2(\alpha_2)$), hence they can be confidently considered as multivariate outliers. This confirms what we previously noticed in the Chi-squared Q-Q plot. \newline
Note also that some observations lie in the strip delimited by the two lines. Nonetheless, we choose not to consider them as multivariate outliers, taking also in account what we observed in the previous point. Furthermore, we point out that the majority of the observations identified as univariate outliers cannot be considered as multivariate outliers with the only exception of the observation $2$.

Finally, we want to observe that these two states were the last ones to join the federation. Of course we don't claim this to be causal. However, the absense of contiguity with the other states definitely reduces the net flow with the other states, thus reducing the homogenisation factors.
...
First observe that $\Sigma$ is invertible since $\det{\Sigma} = -2\rho^3 - 3\rho^2 + 1$ which is greater than $0$, $\forall \rho \in \left(-1, \frac{1}{2}\right)$. \newline
...
Let $A \deq I - c aa^T$; it holds
    \begin{align*}
        A^{-1} & = (I - c aa^T)^{-1} = \sum_{i = 0}^{\infty} (c aa^T)^i = \\
            & = \sum_{i = 0}^{\infty} c^i (aa^T)^i = I + \sum_{i = 1}^{\infty} c^i (\|a\|^2)^{i - 1} aa^T = \\
            & = I + c aa^T \sum_{i = 1}^{\infty} (c \|a\|^2)^{i - 1} = I + c aa^T \sum_{i = 0}^{\infty} (c \|a\|^2)^i = \\
            & = I + c aa^T \frac{1}{1 - c \|a\|^2} = I + \frac{\rho}{1 + \rho} \frac{1}{1 - 3\frac{\rho}{1 + \rho}} aa^T = \\
            & = I + \frac{\rho}{1 - 2\rho} aa^T.
    \end{align*}
Note that to pass from the third to the fourth line it is necessary to have $c\|a\|^2 < 1$ but since we are considering $\rho \in \left(-1, \frac{1}{2}\right)$ it is always true indeed
	\begin{align*}
...
Thus 
    ...
    \]
We can compute $\Sigma^{-1}$ in many other ways; for example we can suppose that $\Sigma^{-1}$ is of the same form $\Sigma$, i.e.
	\[
        \Sigma^{-1} = y I + k aa^T
    \]
and then observe that $P \deq aa^T$ is such that 
	\[	
		P^2 = aa^Taa^T = \|a\|^2 aa^T = 3 P.
	\]
Therefore, while in principle $\Sigma \Sigma^{-1} = I$ may yield no solution since we have a second degree polynomial in $P$ on l.h.s., as $P$ has this property, we have that the l.h.s. is actually of first degree in $P$ so the chances increase. \newline
We can then find the values for $y$ and $k$: 
    \begin{align*}
        \Sigma\Sigma^{-1} & = ((1 + \rho)I - \rho aa^T)(yI + kaa^T) = \\
            & = (1 + \rho)yI + (1 + \rho)kaa^T - \rho yaa^T - k\rho aa^Taa^T = \\
            & = (1 + \rho)yI + ((1 + \rho)k - \rho y - 3 k\rho)aa^T.
    \end{align*}
...
We find the eigenvalues of $\Sigma$ by computing the roots of the characteristic polynomial $p(\lambda) = \det{\Sigma - \lambda I}$.
    \begin{align*}
        \det{\Sigma - \lambda I} & = 
            ...
            & = (1 - \lambda + \rho)(1 - \lambda - \rho - \lambda + \lambda^2 + \lambda \rho - 2\rho^2) = \\
            & = (1 - \lambda + \rho)\left(\lambda^2 + \lambda(\rho - 2)- 2\rho^2 - \rho + 1\right)
    \end{align*}
Hence
    \begin{align*}
...
We denote with $\spectrum{\Sigma}$ the spectrum of the matrix $\Sigma$ (as a linear operator).
    \begin{align*}
    ...
    \end{align*}
Since $(aa^T)a = \|a\|^2 a$ and $\rank{aa^T} = 1$, it holds that
    \[
...
First we need to write the eigenvalues of $\Sigma$ in ascending order. We can distinguish the following two cases:
\begin{enumerate}
    \item 
    ...
    \item
        if $\rho \in (-1, 0)$ then $1 + \rho \leq 1 - 2\rho$. This leads to
            \[
                ...
                \text{ , with } \lambda_1 \geq \lambda_2 \geq \lambda_3.
            \]
        By using the same argument we used in the previous point we obtain that $\rho$ has to satisfy the following condition:
            \[
                (1 - 2\rho) + (1 + \rho) > \frac{4}{5} 3 \iff 2 - \rho > \frac{12}{5} \iff \rho < -\frac{2}{5}.	
            \]
\end{enumerate}
Hence, if $\rho \in \big[0, \frac{1}{2}\big)$, it must be $\rho > \frac{1}{5}$, while, if $\rho \in (-1, 0)$, it must be $\rho < -\frac{2}{5}$. \newline
Finally, $\forall \rho \in \left(-1, -\frac{2}{5}\right) \cup \left(\frac{1}{5}, \frac{1}{2}\right)$ PC1 and PC2 account for more than $80\%$ of the total variation of $Z$.
...
By applying the proposition to
    \begin{gather*}
        ...
        \Sigma = 
            \left(
                \begin{array}{@{}c|c@{}}
                    \var{X} & \begin{matrix} \cov{X}{Y_1} & \cov{X}{Y_2} \end{matrix} \\
                    \vspace{-0.3cm} \\
                    \hline
                    \vspace{-0.3cm} \\
                    \begin{matrix} \cov{X}{Y_1} \\ \cov{X}{Y_2} \end{matrix} & \var{Y}
                \end{array}
            \right) \deq
            \left(
                \begin{array}{@{}c|c@{}}
...
By substituting what we found we get
    \begin{align*}
        \tilde{\mu} & = \mean{\begin{matrix} Y_1 \\ Y_2 \end{matrix}} + \begin{pmatrix} \cov{X}{Y_1} \\ \cov{X}{Y_2} \end{pmatrix} \frac{x - \mean{X}}{\var{X}} = \\
...
and
    \begin{align*}
        \tilde{\Sigma} & = \var{Y} - \frac{1}{\var{X}} \begin{pmatrix} \cov{X}{Y_1} \\ \cov{X}{Y_2} \end{pmatrix} \begin{matrix} \big(\cov{X}{Y_1} & \cov{X}{Y_2} \big) \\ & \end{matrix} = \\
...
An explicit computation reveals
    \[
        c = `r c`. 
    \]
This ellipse is centred in $\mu_Y$, its axes have length $c\sqrt{\lambda_1}$, $c\sqrt{\lambda_2}$ and directions $e_1$, $e_2$, where $(\lambda_1, e_1)$ and $(\lambda_2, e_2)$ are the eigenpairs of the matrix $\Sigma_y$.
...
First of all we divide each variable by \texttt{weight} in order to equalize out the different types of servings of each food.
...
The first three principal components explain the $83.3\%$ of the variability of the data, and if we add a fourth component we arrive near to the $95\%$, which exceeds our requirements.
Hence, it seems reasonable to retain only the first three PCs.

A robustness check for this proposed number of PCs can be performed by looking at the eigenvalues associated to each component.
In order to identify the ones which capture most of the variance of the original variables, we want to identify those that are sized above average. As the original data was previously standardized,  this means to look for eigenvalues greater than $1$. 
...
The scree plot is not so easy to interpret. Due to the absence of an evident \textit{elbow} in the curve, it would not be a useful selection criterium. In conclusion, as only the eigenvalues corresponding to the first three PCs are greater than $1$, we can confirm the choice of retaining only the first three PCs.
...
\begin{enumerate}
    \item[\textbf{PC1.}] 
        Except for the loading associated to the variable \textit{carbohydrates}, which is almost negligible, the other loadings are all negative and lie in the interval $[-0.24, -1]$. The most influential variables are \textit{fat}, \textit{saturated.fat} and \textit{food.energy}, which are related to how much a food is dietetic (or not). Since these loadings are negative, we choose to interpret the first PC as a measure of the dietary power of a certain food item.
    \item[\textbf{PC2.}] 
        The loadings of the second PC are more complicated to analyse. The most influential in a positive sense are the ones corresponding to the variables \textit{carbohydrates} and \textit{food.energy}, while the negative ones are \textit{protein} and \textit{cholesterol}.
        Hence, PC2 identifies how intense in carbohydrates a serving is, and at the same time low in cholesterol. A possible interpretation could be that the component is an indicator of the extent to which a food is composed of grains.
\end{enumerate}

Here we report the scatter plot of our dataset projected on the plane PC1 \textit{vs} PC2.
...
In order to identify univariate outliers, we display the box plots with respect to the first three principal components. 
...
We identified the most extreme (which are displayed in red) by scaling our PCs and then comparing them with some quantiles (with levels over $0.9999$) of a $\mathcal{N}\left(0, 1\right)$. It is remarkable that some outliers have multiplicity equal to $2$: note that in the first box plot the outliers are actually six.

We now want to measure how these outliers score in the original variables. We build a data frame with \textit{min}, \textit{mean} and \textit{max} of each variable. 
...
In the figure below we report a 3D scatter plot of the first three principal components.
...
The outliers found in the previous point are labelled and highlighted in red. The plot shows that the joint distribution of these principal components has high variance. Nevertheless, the red points seem to be significantly far from the cloud. We hypothesise that they could be multivariate outliers, as it will be discussed in more detail in the following.

## 3.6

We investigate multivariate normality through the first three principal components by computing the Mahalanobis distances, and then comparing them with the quantiles of a $\chi^2_3$ with a Q-Q plot. 
...
The Mahalanobis distance is not only of great use to asses the normality of the sample. It can also be employed to detect multivariate outliers. The following plot shows the squared Mahalanobis distances. We also displayed the quantiles of a $\chi^2_3$ for the levels $\alpha = 0.95$ and $\alpha = 0.999$.
...
It can be of interest to asses whether these outliers were also multivariate outliers with respect to the original variables. Therefore, we also plot the corresponding squared Mahalanobis distances, with the quantiles of a $\chi^2_6$ of levels $\alpha = 0.95$ and $\alpha = 0.999$.
...
Not surprisingly, these three outliers are indeed multivariate outliers also for the joint distribution of the original variables. It is worth to remark that also observations $429$, $438$, $439$, $440$ (marked with \textcolor{paleacquamarine}{$\bullet$}) turn out to be multivariate outliers for the original variables. However, they are not so with respect to the three principal components.