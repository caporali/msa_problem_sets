...
# setwd("D:/Università/SDS/Multivariate statistical analysis/problemset_2/problemset_2")
...
Moreover we can test the multivariate normality through statistical tests to be able to state with greater accuracy if our hypotheses are satisfied. In particular we use the Mardia test implemented in \texttt{R} inside the library \texttt{MVN}: \texttt{mvn(\_, mvnTest = "mardia")}.

\smallskip
```{r, render = lemon_print}
test_gws = mvn(data = gws, mvnTest = "mardia")
test_gws$multivariateNormality$Test = c("mardia_skewness", "mardia_kurtosis", "mvn")
names(test_gws$multivariateNormality) = c("test", "stat", "p_value", "result")
test_gws$multivariateNormality$result = tolower(test_gws$multivariateNormality$result)
test_gws$multivariateNormality
```

The results show that our data appear to fail the multivariate gaussianity test. However this could be due to the presence of some possible outliers. What we do is try to remove them and retry the test.
In order to  identify the outliers of the multivariate distribution, we plot the vector of the squared Mahalanobis distances and then add two threshold lines corresponding to different levels of the theoretical quantiles of a $\chi_{24}^2$. In particular, we used $\alpha_1 = 0.95$ (in red (\textcolor{red}{$\blacksquare$})) and $\alpha_2 = \frac{(n - 0.5)}{n}$ with $n = \texttt{nrow(gws)}$ (in blue (\textcolor{blue}{$\blacksquare$})).
...
names(test_gws_out$multivariateNormality) = c("test", "stat", "p_value", "result")
...
We can therefore conclude that it is reasonable to assume that our variables are normally distributed since the presence of $2$ outliers on $`r nrow(gws)`$ is amply justifiable considering the source of our data (i.e. some tests).
...
<!-- todo: add plot of factor loadings -->
...
Finally, in order to analyse better the distribution of our data we decided to display the ellipsoids containing the $95\%$ and the $\frac{n - 0.5}{n}\%$ of the mass of a gaussian distribution with means the sample mean and covariance matrix the sample covariance matrix (with $n$ number of observations we are considering). We can see that the ellipsoid are in fact circles (nearly) which confirms that the first two common factors are jointly normally distributed.
...
names(test_pas$multivariateNormality) = c("test", "stat", "p_value", "result")
test_pas$multivariateNormality$result = tolower(test_pas$multivariateNormality$result)
test_pas$multivariateNormality
```

As before the data initially fails the test but by removing the most extreme outlier (i.e. the $43$\textsuperscript{rd} student) we obtain:
...
names(test_pas_out$multivariateNormality) = c("test", "stat", "p_value", "result")
...
The first derivation further assumes that, for all groups, data are distributed as multivariate gaussian, with different means (the \textit{centroids}) and pooled covariance matrix. Under this assumption, the LDA classifier is the bayes classifier and thus is optimal.
If we now relax the assumption of gaussianity, the centroids and the pooled covariance matrix $\boldsymbol{\Sigma}_{X}$ do not fully identity the distribution within each group anymore. However, we can still recover the LDA classifier as solution to a different problem. Consider the matrix 
\[
