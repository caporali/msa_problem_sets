...
We now consider only the first two discriminant variables (LD1 and LD2) and we perform a scatterplot of the data restricted on these two components, color coding the observations according to variable \texttt{digit\_col} and adding the centroids for each class.
...
<!--
	todo: it might be the case to also add a legenda. 
-->

The most difficult to discriminate are brown = 1, blue = 5, and cyan = 9.

By looking at the plot, we can say that although the centroids are separated from one another (more or less distant), there is a considerable overlap between the classes. In particular, the ones corresponding to the digits $5$, $1$ and $9$ seem to be the most difficult to discriminate as they mix with almost all the others.
...
The plot confirms what we observed before: the classes corresponding to the digits $5$ (blue), $9$ (cyan) and $1$ (brown) 

We have also tried to compute in a more theoretical way the centroids of our $9$ classes. First we have collected in a $16 \times 10$ matrix the means of our classes, then we have computed a \textit{grand mean} by multiplying this matrix for the vector containing the proportion of observations for each class (\texttt{prop\_dig}).
Finally we have \textit{centered} the mean vector of each class by subtracting the grand mean and we have applied the linear trasformation given by the \texttt{scaling} given as output by the \texttt{lda} procedure.
...
What we obtained was the $10 \times 2$ matrix \texttt{centroids\_other}, practically identical to the first two columns of the previously computed matrix \texttt{centroids}. Indeed the uniform norm of the difference of the two distinct computation is $`r norm_diff`$.
...
plot_confusion_matrix(conf_mat$"Confusion Matrix"[[1]],
	class_order = as.character(9:0), add_normalized = F, palette = "Green",
...
We observe that LDA yields the following value for train \textit{MSE} (mean square error):
