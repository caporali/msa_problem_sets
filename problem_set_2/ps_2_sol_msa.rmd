...
Let us first note that we removed 

\smallskip
```{r, fig.align = "center", out.width = "75%"}
sub_digits_5_var = sub_digits[[5]][, 1:15]
d = mahalanobis(sub_digits_5_var, center = colMeans(sub_digits_5_var), cov = cov(sub_digits_5_var))
...
for (i in 1:10){
	centroids_other[i, ] = t(t(lda_fit$scaling[, 1:2]) %*% (as.matrix(colMeans(sub_digits[[i]])) - grand_mean))
}
```

```{r, fig.align = "center", fi}
par(family = "serif")
...
points(centroids_other[, 1], centroids_other[, 2], cex = 1, bg = lookup, pch = 21)
```

```{r}
centroids_other - centroids[, 1:2]
```
